...
export function getSeasonRoute(kind: EpisodicKind, slug: string, season: number): string {
  return `/${kind}/${slug}/season/${season}`;
}

export function getRouteSlug(title: Title, kind: EpisodicKind): string {
  const prefix = `/${kind}/`;
  if (title.url?.startsWith(prefix)) {
    const segment = title.url.slice(prefix.length).split('/').filter(Boolean)[0];
    if (segment) {
      try {
...
export function getTitleRoute(title: Title, kind: EpisodicKind): string {
  return `/${kind}/${getRouteSlug(title, kind)}`;
}

export function getSeasonRouteForTitle(title: Title, kind: EpisodicKind, season: number): string {
  return getSeasonRoute(kind, getRouteSlug(title, kind), season);
}

export function getEpisodeAnchor(episode: number): string {
...
export function getEpisodeRoute(
  ...
  // تم دمج صفحات الحلقات داخل صفحة الموسم لتقليل عدد صفحات Astro المبنية.
  // لذلك أي رابط حلقة يشير الآن إلى بطاقة الحلقة داخل صفحة الموسم بدل صفحة /episode/ منفصلة.
  return `${getSeasonRoute(kind, slug, season)}#${getEpisodeAnchor(episode)}`;
}
...
export function getEpisodeRouteForTitle(
  ...
  return getEpisodeRoute(kind, getRouteSlug(title, kind), season, episode);
}
...
export function getEpisodeWatchRoute(title: Title, season: number, episode: number): string {
  return `/watch/${title.slug}?s=${season}&e=${episode}`;
}
